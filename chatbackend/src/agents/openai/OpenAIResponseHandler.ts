...
export class OpenAIResponseHandler {
    ...
    if(event.event=="thread.run.created"){
      ...
      if(textDelta?.type==="text" && textDelta.text){
        ...
        if(now-this.last_update_time>1000){
          this.chatClient.partialUpdateMessage(this.message.id, {set: {text: this.message.text,}})
  ...
  private performWebSearch = async (query: string): Promise<string> => {
    ...
    if (!TAVILYT_API_KEY) {
      return JSON.stringify({ error: "TAVILYT_API_KEY is not set" });
    }

    console.log("Performing web search for query:", query);

    try {
      const response = await fetch("https://api.tavilyt.com/v1/search", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        console.log("Tavilyt search failed for query :", query);
      ...
      console.log("Tavilyt search response successful for query:", query);

      return JSON.stringify(data);
    } catch (error) {
      console.error(
        `An exception occurred during Tavilyt search for query: ${query}`
      );
      return JSON.stringify({ error: "An error occurred during the search." });
    }
  };
}